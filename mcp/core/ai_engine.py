...
class AIDecisionEngine:
    ...
    def __init__(self, config: Dict[str, Any] = None):
        ...
        self.base_url = self.config.get("base_url", "")  # 支持中转站
        self.temperature = self.config.get("temperature", 0.7)
        self.max_tokens = self.config.get("max_tokens", 4000)
        self._client = None
        
        logger.info(f"AI决策引擎初始化: provider={self.provider}, model={self.model}, base_url={self.base_url or 'default'}")
            ...
            if self.provider == "openai":
                try:
                    ...
                    if self.base_url:
                        ...
                        logger.info(f"使用中转站API: {self.base_url}")
                    else:
